...
# POO: Clase
# POO: Relación entre clases -> Agregación: mantiene listas/objetos de
# Estudiante, Curso, Matricula, Tarea, Calificacion, etc.
class RepositorioAcademico:
    ...
    # POO: Constructor
    # POO: Inyección de dependencias -> recibe "storage: IRepositorio" en vez
    # de crear un JsonManager() internamente; así se puede sustituir por
    # otra implementación (BD, memoria, etc.) sin tocar esta clase
    def __init__(self, storage: IRepositorio):
        ...
        self.reportes_generados: list = []   # RegistroReporte (historial de reportes)
    ...
    # ── Carga ─────────────────────────────────────────────────────────────────
    # Patrón: Factory Method -> aquí se usa UsuarioFactory.crear_usuario() en
    # vez de llamar directamente a Estudiante()/Docente()/etc.
