...
from importacion.facade import ImportacionEstudiantesFacade
...
def construir_tab(parent, repo, usuario_actual, puede_crear: bool):
    ...
    # ── Importación masiva CSV/Excel (solo Administrador) ──────────────────
    def importar_masivo():
        ruta = filedialog.askopenfilename(
            title="Importar estudiantes",
            filetypes=[("Archivos CSV/Excel", "*.csv *.xlsx"), ("CSV", "*.csv"), ("Excel", "*.xlsx")]
        )
        if not ruta:
            return
        try:
            # Patrón: Facade en acción -> internamente combina Factory Method
            # (elige lector CSV/Excel) y Strategy (valida y crea estudiantes)
            resultado = ImportacionEstudiantesFacade(repo).importar_estudiantes_desde_archivo(ruta)
        except Exception as exc:
            messagebox.showerror("Error de importación", str(exc))
            return

        resumen = (
            f"Total de filas: {resultado.total}\n"
            f"Válidas: {resultado.validos}\n"
            f"Inválidas: {resultado.invalidos}\n"
            f"Creadas: {resultado.creados}"
        )
        if resultado.errores:
            detalle = "\n\nErrores principales:\n" + "\n".join(
                f"Fila {e.fila} | {e.campo}: {e.mensaje}" for e in resultado.errores[:10]
            )
            messagebox.showwarning("Importación terminada con errores", resumen + detalle)
        else:
    ...
    if puede_crear:
        ...
        btn(form, "📥  Importar CSV/Excel", C["accent"], importar_masivo, padx=10, pady=5).pack(side="left", padx=4)
