...
from services.academico import TareaService, CalificacionService, RetiroService
...
def construir(parent, usuario, repo):
    ...
    tabs = [
        ...
        ("  📤 Retiro de Materia  ", lambda f: _tab_retiro(f, usuario, repo, rs)),
...
def _tab_reporte(parent, usuario, repo):
    """Reporte de progreso individual del estudiante.

    Delega en DirectorReportes (Facade), que internamente usa
    `services/reporte_service/reporte_estudiante.py` para construir
    el producto Reporte.
    """
    ...
    lbl(top, "Generado mediante DirectorReportes (patrón Facade)",
...
# ── Retiro de Materia ─────────────────────────────────────────────────────────
def _tab_retiro(parent, usuario, repo, rs):
    """Permite al estudiante solicitar el retiro de una materia matriculada.

    La solicitud queda "Pendiente" hasta que el Coordinador la
    apruebe o rechace desde su propio panel.
    """
    wrap = tk.Frame(parent, bg=C["bg"]); wrap.pack(fill="both", expand=True)
    _, sf = scroll_canvas(wrap)

    seccion_titulo(sf, "Solicitar Retiro de Materia")
    lbl(sf, "Puede solicitar el retiro de una asignatura en la que esté "
            "matriculado activamente. La solicitud será revisada por "
            "el Coordinador.", size=9, color=C["text_sec"],
        ).pack(anchor="w", padx=14, pady=(0, 10))

    # Asignaturas matriculadas activas
    activas = repo.matriculas_activas_estudiante(usuario.email)

    form = tk.Frame(sf, bg=C["card"], bd=1, relief="solid", padx=16, pady=12)
    form.pack(fill="x", padx=14, pady=(0, 14))

    lbl(form, "Asignatura matriculada *:", bold=True, bg=C["card"]).pack(anchor="w")
    asig_v = tk.StringVar()
    asig_vals = [f"{m.asignatura.nombre}  (ID:{m.asignatura.id})" for m in activas]
    asig_cb = ttk.Combobox(form, textvariable=asig_v, values=asig_vals,
                           state="readonly", width=50)
    asig_cb.pack(fill="x", ipady=3, pady=(2, 10))

    lbl(form, "Motivo de la solicitud *:", bold=True, bg=C["card"]).pack(anchor="w")
    motivo_t = tk.Text(form, height=4, font=("Segoe UI", 10),
                       bg=C["white"], relief="solid", bd=1)
    motivo_t.pack(fill="x", pady=(2, 10))

    err_lbl = tk.Label(form, text="", fg=C["danger"], bg=C["card"], font=("Segoe UI", 9))
    err_lbl.pack(anchor="w")

    def enviar():
        sel = asig_v.get()
        if not sel:
            err_lbl.config(text="Seleccione una asignatura."); return
        motivo = motivo_t.get("1.0", tk.END).strip()
        if not motivo:
            err_lbl.config(text="Ingrese el motivo de su solicitud."); return
        try:
            mid = int(sel.split("ID:")[1].rstrip(")").strip())
        except (ValueError, IndexError):
            err_lbl.config(text="Asignatura inválida."); return
        ok, msg = rs.solicitar(usuario.email, mid, motivo)
        if ok:
            messagebox.showinfo("✅", msg)
            motivo_t.delete("1.0", tk.END)
            err_lbl.config(text="")
            refrescar_historial()
        else:
            err_lbl.config(text=msg)
    ...
    if not activas:
        lbl(form, "No tiene asignaturas matriculadas activas.",
            size=9, color=C["text_sec"], bg=C["card"]).pack(anchor="w")
    else:
        btn(form, "📤  Enviar solicitud", C["warning"], enviar, pady=8).pack(fill="x")

    # Historial de solicitudes propias
    seccion_titulo(sf, "Mis Solicitudes de Retiro")
    cols = ("ID", "Asignatura", "Motivo", "Fecha", "Estado", "Respuesta del Coordinador")
    tabla, _ = tree_con_scroll(sf, cols, [40, 180, 220, 140, 100, 220], alto=8)

    def refrescar_historial():
        filas = []
        for s in rs.del_estudiante(usuario.email):
            asig = next((a.nombre for a in repo.asignaturas if a.id == s.materia_id), "–")
            filas.append((s.id, asig, s.motivo, s.fecha, s.estado,
                           s.respuesta_coordinador or "-"))
        refrescar_tree(tabla, filas)
