...
# ── Renderizado genérico del producto Reporte ──────────────────────────────────
def render_reporte(parent, reporte, color_header=None):
    """Pinta un objeto `Reporte` (models.reporte.Reporte) en `parent`.

    Es deliberadamente genérico: no le importa qué módulo de
    `services/reporte_service/` construyó el reporte, solo recorre sus
    secciones y totales y los dibuja con los widgets compartidos
    (tarjetas, tablas con scroll).
    """
