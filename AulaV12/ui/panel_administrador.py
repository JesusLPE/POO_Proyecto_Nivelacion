...
def construir(parent, usuario, repo):
    ...
    tabs = [
        ...
        ("  🎓 Estudiantes  ",    lambda f: pge.construir_tab(f, repo, usuario, puede_crear=True)),
        ...
        ("  📄 Auditoría  ",      lambda f: _tab_auditoria(f, repo)),
        ("  🗂️ Reportes  ",       lambda f: _tab_reportes_generados(f, repo)),
...
def _tab_usuarios(parent, repo, us, usuario_actual):
    ...
    def nuevo():
        ...
        rol_v = tk.StringVar(value="docente")
        for r in ("docente", "coordinador", "administrador"):
            tk.Radiobutton(body, text=r.capitalize(), variable=rol_v, value=r,
                           bg=C["bg"], font=("Segoe UI", 10),
                           activebackground=C["bg"]).pack(anchor="w")
        lbl(body, "Para registrar estudiantes use la pestaña 'Estudiantes' "
                  "(requiere asignar curso y horario).",
            size=8, color=C["text_sec"]).pack(anchor="w", pady=(4, 0))
...
def _tab_facultades(parent, repo):
    ...
    lbl(wrap, "Este catálogo se usa al registrar estudiantes y al crear grupos por carrera.",
        color=C["text_sec"]).pack(anchor="w", pady=(8, 0))
...
# ── Auditoría (Reporte del administrador) ─────────────────────────────────────
def _tab_auditoria(parent, repo):
    """Reporte de auditoría del sistema, construido con el patrón Builder.

    Delega en DirectorReportes, que internamente usa
    ReporteAdministradorBuilder para construir el producto Reporte:
    usuarios activos por rol e inconsistencias de datos.
    """
    ...
    top.pack(fill="x")
    seccion_titulo(top, "Reporte de Auditoría del Sistema")
    lbl(top, "Generado mediante ReporteAdministradorBuilder (patrón Builder)",
    ...
    def generar():
        ...
        render_reporte(contenido, reporte, color_header=C["danger"])

    btn(top, "🔄  Generar reporte", C["danger"], generar, padx=10, pady=5).pack(anchor="w", pady=(6, 0))
    generar()


# ── Reportes generados (historial del patrón Builder) ─────────────────────────
def _tab_reportes_generados(parent, repo):
    """Historial de reportes generados por otros roles (p. ej. Coordinador).

    Consulta `ReporteService.historial()`, que lee los `RegistroReporte`
    persistidos en data/reportes_generados.json cada vez que alguien
    genera un reporte mediante el patrón Builder.
    """
    wrap = tk.Frame(parent, bg=C["bg"], padx=12, pady=8)
    wrap.pack(fill="both", expand=True)
    seccion_titulo(wrap, "Reportes Generados por Otros Roles")
    lbl(wrap, "Cada vez que el Coordinador (u otro rol) genera un reporte, "
              "queda registrado aquí para auditoría.",
        size=9, color=C["text_sec"]).pack(anchor="w", pady=(0, 8))

    cols = ("ID", "Tipo", "Título", "Generado por", "Rol", "Fecha", "Resumen")
    t, _ = tree_con_scroll(wrap, cols, [40, 100, 220, 200, 100, 140, 260], alto=12)

    reporte_service = ReporteService(repo)

    def refrescar():
        filas = []
        for r in reporte_service.historial():
            filas.append((r.id, r.tipo.capitalize(), r.titulo,
                           r.generado_por, r.rol_generador.capitalize(),
                           r.fecha, r.resumen_totales))
        refrescar_tree(t, filas)
        if not filas:
            lbl(wrap, "Aún no se han generado reportes desde otros roles.",
                size=9, color=C["text_sec"]).pack(anchor="w", pady=6)

    refrescar()
    btn(wrap, "🔄  Actualizar", C["text_sec"], refrescar, padx=10, pady=5).pack(anchor="w", pady=8)
