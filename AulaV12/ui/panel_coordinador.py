...
from services.academico import CursoService, HorarioService, MatriculaService, RetiroService, CronogramaService
import ui.panel_gestion_estudiantes as pge
...
def construir(parent, usuario, repo):
    ...
    rs = RetiroService(repo)
    crs = CronogramaService(repo)
    ...
    tabs = [
        ...
        ("  🗓️ Cronogramas  ",     lambda f: _tab_cronogramas(f, repo, crs)),
        ("  👨‍🏫 Docentes  ",        lambda f: _tab_docentes(f, repo, cs)),
        ("  🧑‍🎓 Estudiantes  ",     lambda f: pge.construir_tab(f, repo, usuario, puede_crear=False)),
        ("  📋 Matrículas  ",      lambda f: _tab_matriculas(f, repo, ms)),
        ("  📤 Retiros  ",         lambda f: _tab_retiros(f, repo, rs)),
        ("  📊 Reportes  ",        lambda f: _tab_reportes(f, repo, usuario)),
...
def _tab_horarios(parent, repo, hs):
    ...
    def refrescar():
        # Todos los horarios viven en repo.horarios (incluidos los de cada
        # grupo, creados automáticamente al crear el curso). Ya no se generan
        # filas sintéticas "C{id}" a partir de Curso.horario.
        filas = []
...
# ── Reportes ──────────────────────────────────────────────────────────────────
def _tab_reportes(parent, repo, usuario):
    """Reporte de métricas globales y rendimiento, vía patrón Builder.

    Delega en DirectorReportes, que internamente usa
    ReporteCoordinadorBuilder para construir el producto Reporte. Cada
    generación se registra con ReporteService para que el
    Administrador pueda consultarla desde su propia interfaz.
    """
    ...
    top.pack(fill="x")
    seccion_titulo(top, "Reporte de Métricas Globales y Rendimiento")
    lbl(top, "Generado mediante ReporteCoordinadorBuilder (patrón Builder)",
        size=9, color=C["text_sec"]).pack(anchor="w")

    aviso_lbl = lbl(top, "", size=9, color=C["success"])
    aviso_lbl.pack(anchor="w")
    ...
    def generar():
        ...
        render_reporte(contenido, reporte, color_header=C["sidebar2"])
        ...
        aviso_lbl.config(
            text=f"✅  Reporte #{registro.id} guardado en el historial – "
                 f"visible para el Administrador.")

    btn(top, "🔄  Generar reporte", C["sidebar2"], generar, padx=10, pady=5).pack(anchor="w", pady=(6, 0))
    generar()


# ── Solicitudes de Retiro ──────────────────────────────────────────────────────
def _tab_retiros(parent, repo, rs):
    """Bandeja de solicitudes de retiro de materia enviadas por estudiantes.

    El Coordinador puede aprobar o rechazar cada solicitud. Al aprobar,
    RetiroService anula automáticamente la matrícula correspondiente.
    """
    wrap = tk.Frame(parent, bg=C["bg"], padx=12, pady=8)
    wrap.pack(fill="both", expand=True)
    seccion_titulo(wrap, "Solicitudes de Retiro de Materia")

    # Filtro
    fil_f = tk.Frame(wrap, bg=C["bg"]); fil_f.pack(fill="x", pady=(0, 8))
    lbl(fil_f, "Estado:", bold=True).pack(side="left")
    fil_v = tk.StringVar(value="Pendiente")
    for op in ("Pendiente", "Aprobada", "Rechazada", "Todas"):
        tk.Radiobutton(fil_f, text=op, variable=fil_v, value=op,
                       bg=C["bg"], font=("Segoe UI", 9),
                       command=lambda: refrescar()).pack(side="left", padx=8)

    cols = ("ID", "Estudiante", "Carrera", "Asignatura", "Motivo", "Fecha", "Estado")
    t, _ = tree_con_scroll(wrap, cols, [35, 170, 150, 180, 220, 140, 90], alto=11)

    def refrescar():
        filtro = fil_v.get()
        filas = []
        for s in rs.todas():
            if filtro != "Todas" and s.estado != filtro: continue
            est = repo.usuarios.get(s.estudiante_id)
            asig = next((a.nombre for a in repo.asignaturas if a.id == s.materia_id), "–")
            carrera = getattr(est, "carrera", "") if est else "–"
            filas.append((s.id, est.nombre_completo() if est else s.estudiante_id,
                           carrera, asig, s.motivo, s.fecha, s.estado))
        refrescar_tree(t, filas)
    refrescar()

    form = tk.Frame(wrap, bg=C["bg"]); form.pack(fill="x", pady=6)

    def _resolver(aprobar: bool):
        sel = t.selection()
        if not sel: messagebox.showerror("Error", "Seleccione una solicitud."); return
        sid = t.item(sel[0])["values"][0]
        accion = "aprobar" if aprobar else "rechazar"
        respuesta = simpledialog.askstring(
            "Respuesta al estudiante",
            f"Comentario para el estudiante (opcional) al {accion} la solicitud:")
        if respuesta is None:
            return  # canceló el diálogo
        ok, msg = rs.resolver(sid, aprobar, respuesta or "")
        (messagebox.showinfo if ok else messagebox.showerror)("Resultado", msg)
        if ok: refrescar()

    btn(form, "✅  Aprobar",  C["success"], lambda: _resolver(True),  padx=10, pady=5).pack(side="left", padx=4)
    btn(form, "🚫  Rechazar", C["danger"],  lambda: _resolver(False), padx=10, pady=5).pack(side="left", padx=4)
    btn(form, "🔄  Actualizar", C["text_sec"], refrescar, padx=10, pady=5).pack(side="left", padx=4)


# ── Cronogramas ─────────────────────────────────────────────────────────────────
def _tab_cronogramas(parent, repo, crs):
    """Gestión del cronograma académico (fechas y carga horaria) por grupo.

    Cronograma es un modelo de dominio puro (models/cronograma.py),
    persistido vía CronogramaService.
    """
    wrap = tk.Frame(parent, bg=C["bg"], padx=12, pady=8)
    wrap.pack(fill="both", expand=True)
    seccion_titulo(wrap, "Cronogramas Académicos por Grupo")

    cols = ("ID", "Grupo", "Inicio", "Fin", "Total horas", "Descripción")
    t, _ = tree_con_scroll(wrap, cols, [40, 220, 100, 100, 90, 220], alto=11)

    def refrescar():
        filas = []
        for c in crs.todos():
            curso = repo.curso_por_id(c.curso_id)
            filas.append((c.id, curso.nombre if curso else f"Curso {c.curso_id}",
                           c.fecha_inicio, c.fecha_fin, c.total_horas, c.descripcion or "-"))
        refrescar_tree(t, filas)
    refrescar()

    form = tk.Frame(wrap, bg=C["bg"]); form.pack(fill="x", pady=6)

    def _form_cronograma(titulo, defaults=None):
        v = ventana_modal(wrap.winfo_toplevel(), titulo, 480, 420)
        body = tk.Frame(v, bg=C["bg"], padx=20, pady=16); body.pack(fill="both", expand=True)

        lbl(body, "Grupo *:", bold=True).pack(anchor="w")
        c_vals = [f"{c.nombre}  (ID:{c.id})" for c in repo.cursos]
        c_v = tk.StringVar()
        c_cb = ttk.Combobox(body, textvariable=c_v, values=c_vals,
                            state="readonly" if defaults is None else "disabled", width=54)
        c_cb.pack(fill="x", ipady=3, pady=(2, 8))
        if defaults:
            curso = repo.curso_por_id(defaults["curso_id"])
            c_v.set(f"{curso.nombre}  (ID:{curso.id})" if curso else "")

        fila = tk.Frame(body, bg=C["bg"]); fila.pack(fill="x", pady=(0, 8))
        lbl(fila, "Fecha inicio (YYYY-MM-DD):", bold=True).pack(side="left")
        ini_e = ttk.Entry(fila, width=14); ini_e.pack(side="left", padx=8, ipady=3)
        lbl(fila, "Fecha fin (YYYY-MM-DD):", bold=True).pack(side="left", padx=(12, 0))
        fin_e = ttk.Entry(fila, width=14); fin_e.pack(side="left", padx=8, ipady=3)
        if defaults:
            ini_e.insert(0, defaults.get("fecha_inicio", ""))
            fin_e.insert(0, defaults.get("fecha_fin", ""))

        lbl(body, "Total de horas *:", bold=True).pack(anchor="w")
        horas_e = ttk.Entry(body, width=20); horas_e.pack(anchor="w", ipady=3, pady=(2, 8))
        if defaults: horas_e.insert(0, str(defaults.get("total_horas", "")))

        lbl(body, "Descripción / observaciones:", bold=True).pack(anchor="w")
        desc_e = ttk.Entry(body, width=54); desc_e.pack(fill="x", ipady=3, pady=(2, 14))
        if defaults: desc_e.insert(0, defaults.get("descripcion", ""))

        err_lbl = tk.Label(body, text="", fg=C["danger"], bg=C["bg"], font=("Segoe UI", 9))
        err_lbl.pack(anchor="w")

        return v, body, c_v, ini_e, fin_e, horas_e, desc_e, err_lbl

    def nuevo():
        v, body, c_v, ini_e, fin_e, horas_e, desc_e, err_lbl = _form_cronograma("Nuevo Cronograma")
        ...
        def guardar():
            ...
            try: horas = int(horas_e.get())
            except ValueError: err_lbl.config(text="El total de horas debe ser un número entero."); return
            ok, msg = crs.crear(cid, ini_e.get().strip(), fin_e.get().strip(),
                                horas, desc_e.get().strip())
            if ok:
                messagebox.showinfo("✅", msg); refrescar(); v.destroy()
            else:
                err_lbl.config(text=msg)

        btn(body, "✔  Guardar cronograma", C["success"], guardar, pady=9).pack(fill="x")

    def editar():
        sel = t.selection()
        if not sel: messagebox.showerror("Error", "Seleccione un cronograma."); return
        cid = t.item(sel[0])["values"][0]
        c = next((x for x in crs.todos() if x.id == cid), None)
        if not c: return

        defaults = {"curso_id": c.curso_id, "fecha_inicio": c.fecha_inicio,
                     "fecha_fin": c.fecha_fin, "total_horas": c.total_horas,
                     "descripcion": c.descripcion}
        v, body, _, ini_e, fin_e, horas_e, desc_e, err_lbl = _form_cronograma("Editar Cronograma", defaults)

        def guardar():
            try: horas = int(horas_e.get())
            except ValueError: err_lbl.config(text="El total de horas debe ser un número entero."); return
            ok, msg = crs.editar(cid, ini_e.get().strip(), fin_e.get().strip(),
                                 horas, desc_e.get().strip())
            if ok:
                messagebox.showinfo("✅", msg); refrescar(); v.destroy()
            else:
    ...
    def eliminar():
        ...
        if not sel: messagebox.showerror("Error", "Seleccione un cronograma."); return
        cid = t.item(sel[0])["values"][0]
        if messagebox.askyesno("Confirmar", "¿Eliminar este cronograma?"):
            ok, msg = crs.eliminar(cid)
            (messagebox.showinfo if ok else messagebox.showerror)("Resultado", msg)
            if ok: refrescar()

    btn(form, "➕  Nuevo cronograma", C["success"], nuevo,    padx=10, pady=5).pack(side="left", padx=4)
    btn(form, "✏  Editar",           C["accent"],  editar,   padx=10, pady=5).pack(side="left", padx=4)
    btn(form, "🗑  Eliminar",         C["danger"],  eliminar, padx=10, pady=5).pack(side="left", padx=4)
    btn(form, "🔄  Actualizar",       C["text_sec"],refrescar,padx=10, pady=5).pack(side="left", padx=4)
