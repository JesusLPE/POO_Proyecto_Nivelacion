...
class SistemaAcademicoApp:
    ...
    def __init__(self, repositorio: RepositorioAcademico):
        ...
        self.entregas  = self._obtener_entregas()
    ...
    def _obtener_entregas(self):
        entregas = []
        for tarea in self.tareas:
            for entrega in tarea.entregas:
                estudiante = self.usuarios.get(entrega.get("estudiante_email"))
                entregas.append({
                    "tarea": tarea,
                    ...
                    "estado": entrega.get("estado", "Realizada"),
                })
        return entregas

    def _refrescar_entregas(self):
        self.entregas = self._obtener_entregas()

    def obtener_materias_estudiante(self, estudiante=None):
        estudiante = estudiante or self.current_user
        if not estudiante:
            return []
        return self.repo.obtener_materias_estudiante(estudiante.email)

    def _tarea_entregada(self, tarea, estudiante=None):
        estudiante = estudiante or self.current_user
        if not tarea or not estudiante:
            return False
        return any(
            entrega.get("estudiante_email") == estudiante.email
            and entrega.get("estado") == "Realizada"
            for entrega in tarea.entregas
        )

    def _tarea_entregada_por_titulo(self, titulo):
        tarea = next((t for t in self.tareas if t.titulo == titulo), None)
        return self._tarea_entregada(tarea)
    ...
    def _formatear_fecha_tarea(self, tarea):
        ...
        try:
            ...
            return f"Vencimiento: {fecha.strftime('%Y-%m-%d')}"
        except ValueError:
            return f"Vencimiento: {tarea.fecha_limite}"

    def _tareas_pendientes_estudiante(self):
        return [
            tarea
            for tarea in self.tareas
            if not self._tarea_entregada(tarea)
        ]
    ...
    def show_login(self):
        ...
        tk.Frame(left, bg=C["sidebar"]).pack(expand=True)
        tk.Label(left, text="© 2026 – Proyecto Pogramacion Orientada A Objeto", font=("Segoe UI", 9),
    ...
    def show_timeline(self, parent, rol):
        ...
        if not actividades:
            tk.Label(sf, text="No tienes actividades pendientes.",
                     font=("Segoe UI", 10, "bold"), bg=C["bg"], fg=C["text_sec"]
                     ).pack(pady=18)
            return
            ...
            return [
                {
                    ...
                    "boton_comando": lambda t=tarea: self.subir_tarea(
                        t.titulo,
                        t.descripcion,
                        t.fecha_limite,
                    ),
                }
                for tarea in self._tareas_pendientes_estudiante()
            ]
            proyecto_entregada = self._tarea_entregada_por_titulo("Proyecto Final - Diseno de Algoritmos")
            avance_entregada = self._tarea_entregada_por_titulo("Entrega de Avance - Estructuras de Datos")
            return [
                {"fecha": "Vencimiento: Viernes 22 mayo – 23:00",
                 ...
                 "boton_texto": "Tarea entregada" if proyecto_entregada else "Agregar entrega",
                 "boton_comando": (lambda: self._mostrar_info("Esta tarea ya fue entregada"))
                                  if proyecto_entregada else lambda: self.subir_tarea("Proyecto Final - Diseno de Algoritmos",
                                                            "Entregar documentacion y codigo fuente",
                                                            "2026-05-22")},
                {"fecha": "Sábado 23 mayo – 00:59",
                 "titulo": "📄 Entrega de Avance – Estructuras de Datos",
                 "descripcion": "Implementar árboles binarios",
                 "boton_texto": "Tarea entregada" if avance_entregada else "Agregar entrega",
                 "boton_comando": (lambda: self._mostrar_info("Esta tarea ya fue entregada"))
                                  if avance_entregada else lambda: self.subir_tarea("Entrega de Avance - Estructuras de Datos",
    ...
    def show_cursos(self, parent):
        ...
        cursos_matriculados = self.obtener_materias_estudiante()
        cursos = cursos_matriculados or self.cursos_lista
        ...
        for i, curso in enumerate(cursos):
            ...
            tk.Label(inner, text=curso.nombre, font=("Segoe UI", 11, "bold"),
                     bg=C["card"], fg=col).pack(anchor="w")
            horario = str(curso.horario) if hasattr(curso, "horario") and curso.horario else "Sin horario"
            modalidad = str(curso.modalidad) if hasattr(curso, "modalidad") and curso.modalidad else "Sin modalidad"
            tk.Button(inner, text="Ingresar", bg=col, fg=C["white"],
                      font=("Segoe UI", 9, "bold"), relief="flat", cursor="hand2",
                      padx=10, pady=4, command=lambda c=curso: self.ingresar_curso(c)
    ...
    #  ESTUDIANTE – acciones
    # ══════════════════════════════════════════════════════════
    def ingresar_curso(self, curso):
        v = self._crear_ventana_secundaria(f"Curso: {curso.nombre}", 560, 420)
        body = tk.Frame(v, bg=C["bg"], padx=18, pady=14)
        body.pack(fill="both", expand=True)

        tk.Label(body, text=curso.nombre, font=("Segoe UI", 14, "bold"),
                 bg=C["bg"], fg=C["sidebar"]).pack(anchor="w", pady=(0, 8))
        horario = str(curso.horario) if hasattr(curso, "horario") and curso.horario else "Sin horario"
        modalidad = str(curso.modalidad) if hasattr(curso, "modalidad") and curso.modalidad else "Sin modalidad"
        tk.Label(body, text=f"{horario} | {modalidad}", font=("Segoe UI", 10),
                 bg=C["bg"], fg=C["text_sec"]).pack(anchor="w", pady=(0, 12))

        tk.Label(body, text="Tareas", font=("Segoe UI", 11, "bold"),
                 bg=C["bg"], fg=C["text"]).pack(anchor="w")
        tareas_curso = [t for t in self.tareas if curso.nombre.lower() in t.titulo.lower()]
        if not tareas_curso:
            tareas_curso = self.tareas
        if not tareas_curso:
            tk.Label(body, text="No hay tareas registradas para este curso.",
                     font=("Segoe UI", 10), bg=C["bg"], fg=C["text_sec"]).pack(anchor="w", pady=8)
        for tarea in tareas_curso:
            fila = tk.Frame(body, bg=C["card"], bd=1, relief="solid", padx=10, pady=8)
            ...
            tk.Button(fila, text="Tarea entregada" if entregada else "Entregar",
                      bg=C["success"] if entregada else C["accent"], fg=C["white"],
                      font=("Segoe UI", 9, "bold"), relief="flat",
                      cursor="arrow" if entregada else "hand2",
                      state="disabled" if entregada else "normal",
                      command=lambda t=tarea: self.subir_tarea(
                          t.titulo,
                          t.descripcion,
                          t.fecha_limite,
                          on_success=lambda: (v.destroy(), self.ingresar_curso(curso)),
                      )
                      ).pack(side="right")

    def subir_tarea(self, tarea_titulo=None, tarea_descripcion="", fecha_limite="", on_success=None):
        ...
        body.winfo_children()[-1].pack_forget()
        body.winfo_children()[-2].pack_forget()

        tk.Label(body, text="Descripción:", font=("Segoe UI", 10, "bold"), bg=C["bg"], fg=C["text"]).pack(anchor="w")
        body.winfo_children()[-1].pack_forget()
        if tarea_titulo:
            tarea_previa = next((t for t in self.tareas if t.titulo == tarea_titulo), None)
            if not tarea_previa:
                nuevo_id = max((t.id or 0 for t in self.tareas), default=0) + 1
                tarea_previa = Tarea(tarea_titulo, tarea_descripcion, fecha_limite, "sistema", nuevo_id)
                self.tareas.append(tarea_previa)
                self.repo.guardar_tareas()
        else:
            tarea_previa = None

        tareas_vals = [f"{t.id} - {t.titulo}" for t in self.tareas]
        tarea_v = tk.StringVar()
        if tarea_previa:
            tarea_v.set(f"{tarea_previa.id} - {tarea_previa.titulo}")
        tk.Label(body, text="Tarea existente:", font=("Segoe UI", 10, "bold"), bg=C["bg"], fg=C["text"]).pack(anchor="w")
        ttk.Combobox(body, textvariable=tarea_v, values=tareas_vals, width=55).pack(ipady=4, pady=(2, 10), fill="x")
        tk.Label(body, text="Descripcion de la entrega:", font=("Segoe UI", 10, "bold"), bg=C["bg"], fg=C["text"]).pack(anchor="w")
        ...
        def guardar():
            sel = tarea_v.get()
            if not sel:
                self._mostrar_error("Seleccione una tarea"); return
            if not self.tareas:
                nuevo_id = 1
                tarea = Tarea(sel.strip(), "", "", "sistema", nuevo_id)
                self.tareas.append(tarea)
                tarea_v.set(f"{tarea.id} - {tarea.titulo}")
                sel = tarea_v.get()
                self.repo.guardar_tareas()
            if " - " not in sel:
                tarea = next((t for t in self.tareas if t.titulo == sel.strip()), None)
                if not tarea:
                    nuevo_id = max((t.id or 0 for t in self.tareas), default=0) + 1
                    tarea = Tarea(sel.strip(), "", "", "sistema", nuevo_id)
                    self.tareas.append(tarea)
                ...
                sel = tarea_v.get()
            try:
                id_tarea = int(sel.split(" - ", 1)[0])
            except ValueError:
                self._mostrar_error("Seleccione una tarea valida"); return
            tarea = next((t for t in self.tareas if t.id == id_tarea), None)
            if not tarea:
                self._mostrar_error("Tarea no encontrada"); return
            if self._tarea_entregada(tarea):
                self._mostrar_info("Esta tarea ya fue entregada")
                v.destroy()
                if on_success:
                    on_success()
                else:
            ...
            self.repo.guardar_tareas()
            self._refrescar_entregas()
            self._mostrar_info("Â¡Tarea subida exitosamente!")
            v.destroy()
            if on_success:
                on_success()
            else:
    ...
    def crear_tarea(self):
        ...
        def guardar():
            ...
            nuevo_id = max((t.id or 0 for t in self.tareas), default=0) + 1
            tarea = Tarea(titulo, v.desc_t.get("1.0", tk.END).strip(), fecha, self.current_user.email, nuevo_id)
            self.tareas.append(tarea)
            self.repo.guardar_tareas()
    ...
    def show_entregas(self, parent):
        ...
        tree = ttk.Treeview(body, columns=("Tarea", "Estudiante", "Archivo", "Fecha", "Estado"), show="headings")
        for col in ("Tarea", "Estudiante", "Archivo", "Fecha", "Estado"):
        ...
        for e in self.entregas:
            tree.insert("", "end", values=(
                ...
                e["archivo"], e["fecha"], e.get("estado", "Realizada")
            ))
    ...
    def gestionar_matriculas(self):
        ...
        def refrescar():
            ...
            for m in self.matriculas_lista:
                ...
                asignatura = m.asignatura.nombre if m.asignatura else "N/A"
                tree.insert("", "end", values=(m.id, est, asignatura, m.fecha, m.tipo, m.estado))
        refrescar()
        ...
        tk.Label(form, text="Asignatura:", font=("Segoe UI", 9, "bold"), bg=C["bg"], fg=C["text"]).pack(side="left", padx=4)
        asignatura_v = tk.StringVar()
        ttk.Combobox(
            form,
            textvariable=asignatura_v,
        ...
        def nueva_matricula():
            ...
            sel_asig = asignatura_v.get()
            if not sel_asig: self._mostrar_error("Seleccione una asignatura"); return
            try:
                id_asig = int(sel_asig.split("(")[-1].rstrip(")"))
            except ValueError:
                self._mostrar_error("Seleccione una asignatura valida"); return
            asignatura = next((a for a in self.asignaturas_lista if a.id == id_asig), None)
            if not asignatura: self._mostrar_error("Asignatura no encontrada"); return
