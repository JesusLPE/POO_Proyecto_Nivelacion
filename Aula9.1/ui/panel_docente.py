...
def _tab_reportes(parent, usuario, repo, ts):
    ...
    def datos_acta():
        ...
        return [{"titulo": "Reporte de acta final", "columnas": cols, "filas": filas}]

    def mostrar_acta(secciones):
    ...
    def datos_asistencia():
        ...
        return [{"titulo": "Reporte de asistencia", "columnas": cols, "filas": filas}]
    ...
    def datos_calificaciones():
        ...
        return [{"titulo": "Reporte de calificaciones", "columnas": cols, "filas": filas}]

    def mostrar_calificaciones(secciones):
        cols = secciones[0]["columnas"]
        filas = secciones[0]["filas"]
        t, _ = tree_con_scroll(contenido, cols, [210, 150, 150, 120, 120], alto=14)
        refrescar_tree(t, filas)

    def datos_actividades():
        ...
        return [{"titulo": "Reporte de actividades evaluativas", "columnas": cols, "filas": filas}]

    def mostrar_actividades(secciones):
        cols = secciones[0]["columnas"]
        filas = secciones[0]["filas"]
    ...
    def generar(guardar=False):
        ...
        if guardar:
            ruta = rs.guardar("Docente", usuario.nombre_completo(), tipo, secciones)
