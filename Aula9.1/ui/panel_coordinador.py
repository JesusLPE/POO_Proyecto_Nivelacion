...
from services.reportes import ReporteService
...
def _tab_horarios(parent, repo, hs):
    ...
    def nuevo():
        v = ventana_modal(wrap.winfo_toplevel(), "Programar Horario", 500, 490)
        body = tk.Frame(v, bg=C["bg"], padx=20, pady=16); body.pack(fill="both", expand=True)

        lbl(body, "ID del horario:", bold=True).pack(anchor="w")
        id_e = ttk.Entry(body, width=54)
        id_e.pack(fill="x", ipady=3, pady=(2, 8))
        id_e.insert(0, str(repo.siguiente_id_dict(repo.horarios)))
        ...
        def guardar():
            ...
            try:
                hid = int(id_e.get().strip())
            except ValueError:
                err_lbl.config(text="El ID del horario debe ser un numero entero."); return
            ...
            ok, msg = hs.crear(cid, dia_v.get(), ini_e.get().strip(),
                               fin_e.get().strip(), mod_v.get(), aula_e.get().strip(), hid)
            if ok:
                messagebox.showinfo("✅", msg); refrescar(); v.destroy()
            else:
                err_lbl.config(text=msg)
...
def _tab_reportes(parent, repo):
    ...
    def datos_docentes():
        ...
        return [{"titulo": "Reporte de docentes", "columnas": cols, "filas": filas}]

    def mostrar_docentes(secciones):
        cols = secciones[0]["columnas"]
        filas = secciones[0]["filas"]
        t, _ = tree_con_scroll(contenido, cols, [190, 230, 230, 110], alto=14)
        refrescar_tree(t, filas)

    def datos_estadistico():
        ...
        return [{"titulo": "Reporte estadistico de nivelacion", "columnas": cols, "filas": filas}]

    def mostrar_estadistico(secciones):
        cols = secciones[0]["columnas"]
        filas = secciones[0]["filas"]
    ...
    def datos_rendimiento():
        ...
        return [
            {"titulo": "Promedios por asignatura", "columnas": cols, "filas": filas1},
            {"titulo": "Promedios por paralelo", "columnas": cols2, "filas": filas2},
        ]

    def mostrar_rendimiento(secciones):
        tk.Label(contenido, text="Promedios por asignatura",
                 font=("Segoe UI", 11, "bold"), bg=C["bg"], fg=C["sidebar"]).pack(anchor="w", pady=(0, 4))
        cols = secciones[0]["columnas"]
        filas1 = secciones[0]["filas"]
        ...
        cols2 = secciones[1]["columnas"]
        filas2 = secciones[1]["filas"]
        t2, _ = tree_con_scroll(contenido, cols2, [220, 180, 100, 100], alto=7)
        refrescar_tree(t2, filas2)

    def datos_matriculados():
        ...
        return [{"titulo": "Reporte de estudiantes matriculados", "columnas": cols, "filas": filas}]

    def mostrar_matriculados(secciones):
        cols = secciones[0]["columnas"]
        filas = secciones[0]["filas"]
    ...
    def datos_asistencia():
        ...
        return [{"titulo": "Reporte de asistencia general", "columnas": cols, "filas": filas}]

    def mostrar_asistencia(secciones):
        cols = secciones[0]["columnas"]
        filas = secciones[0]["filas"]
    ...
    def datos_reporte(tipo):
        if tipo == "Docentes":
            return datos_docentes()
        if tipo == "Estadistico de nivelacion":
            return datos_estadistico()
        if tipo == "Rendimiento academico":
            return datos_rendimiento()
        if tipo == "Estudiantes matriculados":
            return datos_matriculados()
        return datos_asistencia()

    def mostrar_reporte(tipo, secciones):
        if tipo == "Docentes":
            mostrar_docentes(secciones)
        elif tipo == "Estadistico de nivelacion":
            mostrar_estadistico(secciones)
        elif tipo == "Rendimiento academico":
            mostrar_rendimiento(secciones)
        elif tipo == "Estudiantes matriculados":
            mostrar_matriculados(secciones)
        else:
            mostrar_asistencia(secciones)

    def mostrar_espera(_=None):
        limpiar()
        estado_lbl.config(text="")
        tk.Label(contenido,
                 text="Seleccione un tipo de reporte y presione Generar para crear el archivo.",
                 font=("Segoe UI", 10), bg=C["bg"], fg=C["text_sec"]
                 ).pack(anchor="w", pady=12)

    def generar(guardar=False):
        limpiar()
        ahora = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
        tipo = tipo_v.get()
        secciones = datos_reporte(tipo)
        ...
        mostrar_reporte(tipo, secciones)
        if guardar:
            ruta = rs.guardar("Coordinador", "Coordinador", tipo, secciones)
            messagebox.showinfo("Reporte generado", f"Reporte generado correctamente.\n\nArchivo:\n{ruta}")

    cb.bind("<<ComboboxSelected>>", mostrar_espera)
    btn(filtro, "Generar", C["success"], lambda: generar(True), padx=10, pady=5).pack(side="left", padx=4)
    mostrar_espera()
    return
