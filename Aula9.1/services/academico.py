...
class HorarioService:
    ...
    def crear(self, curso_id: int, dia: str, inicio: str,
              fin: str, modalidad: str, aula: str,
              horario_id: Optional[int] = None) -> Tuple[bool, str]:
        ...
        nid = horario_id if horario_id is not None else self._repo.siguiente_id_dict(self._repo.horarios)
        if nid <= 0:
            return False, "El ID del horario debe ser positivo"
        if any(x.get("id") == nid for x in self._repo.horarios):
            return False, f"Ya existe un horario con ID {nid}"
