...
est1 = Estudiante("Diocles", "Bacusoy", "diocles@gmail.com", "1234", "A001", "Ingeniería")
...
def perfil():
    if request.method == 'POST':
        if current_user.obtener_rol() == 'estudiante':
            # Simulamos la actualización de perfil del estudiante.
...
def subir_tarea():
    if request.method == 'POST':
        ...
        if archivo:
            # Simulación: no almacenamos el archivo en fase 1.
...
@role_required('estudiante')
def mis_tareas():
    # Aún no hay tareas guardadas, solo se muestra el diseño de la página.
    tareas_vacias = []
